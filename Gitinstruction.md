...
## 12. Удаление веток
Чтобы удалить локальную ветку, мы можем испоьзовать команду:
```
git branch (-d | -D) + name branch
```
Опция **-d** это псевдоним для **--delete** которые удаляет ветвь только в том случае если она была полностью объединена с вышестоящей ветвью.


Опция **-D** это псевдоssним для **--delete** (--force), котssорые позволяют удалять ветку незовисимо от ее статуса слиsssяние:
```
