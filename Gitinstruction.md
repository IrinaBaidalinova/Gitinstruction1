...
## 12. Удаление веток
Чтобы удалить локальную aaaaветку, мы можем испоьзовать команду:
```
git branch (-d | -D) + name branchaaaa
```
Опция **-d** это псевдоним для **--delete** которыеaaaa удаляет ветвь только в том случае если она была полностью объединена с вышестоящей ветвью.